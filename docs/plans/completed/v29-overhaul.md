...
- **Legacy monolith** — [`agent/graph.py`](../../../src/agent_first_browse/agent/graph.py): keep only the shared utilities the brain
...
`agent.sh` → `agent-browse run` → [`agent.graph.run_brain`](../../../src/agent_first_browse/agent/graph.py) (LangGraph).
...
**Smart scroll.** [`actions.tools.mcp_scroll`](../../../src/agent_first_browse/actions/tools.py) now measures the viewport before/after the
...
unaffected. Expanded primitives in [actions/tools.py](../../../src/agent_first_browse/actions/tools.py) reusing existing backends:
