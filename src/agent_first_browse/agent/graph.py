...
async def _wait_for_perception_readiness(page, state: BrainState) -> None:
    ...
    if page is None:
        return
    try:
        from agent_first_browse.survey.context import survey_perception_wait_mode
...
async def goal_compiler_node(state: BrainState) -> dict:
    ...
    from agent_first_browse.verification.safety import auto_allow_from_objective
    ...
    from agent_first_browse.survey.context import is_continuous_survey_mission
...
async def planner_node(state: BrainState) -> dict:
    ...
    from agent_first_browse.cognition.reasoning import StrategicPlan, merge_beliefs
    ...
    try:
        from agent_first_browse.survey.context import sanitize_survey_plan
...
async def perceive_node(state: BrainState) -> dict:
    ...
    if state.step_number >= state.max_steps:
        if state.continuous_survey_mode:
            from agent_first_browse.survey.context import rolling_continuous_budget
    ...
    if sparse_dom_status != "NOT_NEEDED":
        ...
        if sparse_dom_status == "UNRESOLVED":
            try:
                from agent_first_browse.survey.context import survey_failure_fingerprint
                logger.warning(
    ...
    if state.continuous_survey_mode and not survey_provider_urls:
        try:
            from agent_first_browse.survey.context import survey_provider_urls as configured_survey_providers
    ...
    if state.continuous_survey_mode or "survey" in state.objective.lower():
        try:
            ...
            from agent_first_browse.survey.context import is_verified_survey_page_transition
            ...
            if completion:
                ...
                if cycle_boundary_pending:
                    from agent_first_browse.survey.context import survey_cycle_cleanup_updates
    ...
    if state.continuous_survey_mode:
        try:
            ...
            if completion:
                try:
                    from agent_first_browse.survey.site_quirks import fresh_dashboard_after_completion
            ...
            if fresh_completion_dashboard:
                ...
                if survey_provider_urls and len(survey_provider_urls) > 1:
                    from agent_first_browse.survey.outcomes import choose_survey_provider_index
    ...
    try:
        from agent_first_browse.survey.audio import audio_challenge_key, analyze_audio_challenge
    ...
    if state.continuous_survey_mode:
        try:
            from agent_first_browse.survey.context import canonical_survey_url
    ...
    if not cycle_context_reset:
        try:
            from agent_first_browse.cognition.stagnation import detect_navigation_cycle
            from agent_first_browse.survey.context import survey_page_fingerprint
    ...
    try:
        from agent_first_browse.survey.context import render_cycle_answer_memory
    ...
    try:
        ...
        if stagnation_enabled():
            ...
            from agent_first_browse.survey.context import has_recent_survey_progress
...
async def commit_node(state: BrainState) -> dict:
    ...
    if state.continuous_survey_mode:
        ...
        if committed_verb == "abandon_survey" and action_ok:
            ...
            try:
                from agent_first_browse.survey.outcomes import get_survey_outcome_store
...
async def finalize_node(state: BrainState) -> dict:
    ...
    if (not state.continuous_survey_mode
        ...
        try:
            from agent_first_browse.verification.overwatch import final_outcome_audit
    ...
    from agent_first_browse.cognition.reasoning import clear_all
...
async def run_brain(objective: str, headless: bool = False):
    ...
    from agent_first_browse.browser.runtime import (
        launch_browser, SessionGuard, shutdown_browser,
    )
    ...
    async def _invoke_with_failover(chain, messages, schema=None, breaker=None, **kwargs):
        """Preserve the legacy graph callback's positional breaker contract."""
        return await _invoke_model(
            chain, messages, schema, breaker=breaker, **kwargs
        )
    ...
    try:
        from agent_first_browse.memory.intent_journal import resolve_intent
        resolve_intent()
    except Exception:
        pass
    ...
    # Preserve the historical repository-level persistence location after the
    # implementation moved under src/agent_first_browse/agent.
    db_path = Path(__file__).resolve().parents[3] / "persistence" / "brain_checkpoints.db"
    ...
    async with AsyncSqliteSaver.from_conn_string(str(db_path)) as checkpointer:
        ...
        try:
            from agent_first_browse.survey.profile import compact_runtime_profile, load_active_profile, render_profile
            durable_profile = load_active_profile()
