...
def _escalate(state: dict, reason: str, updates: dict) -> dict:
    ...
    """
    from agent_first_browse.cognition.reasoning import advance_ladder, update_confidence, obstacle_key
...
def _action_loop_signature(state: dict[str, Any], verb: str, element_id: Any) -> str:
    ...
    try:
        from agent_first_browse.survey.context import survey_semantic_page_identity
...
def _note_survey_no_effect(
    ...
    if not state.get("continuous_survey_mode"):
        return
    try:
        from agent_first_browse.survey.context import survey_action_attempt_key
...
async def overwatch_node(state: dict[str, Any], page, critic, action_verifier=None) -> dict:
    ...
    if verb in {"type", "click", "select_option", "set_date_of_birth"}:
        try:
            from agent_first_browse.survey.context import is_survey_mission
    ...
    applied_site_quirk = ""
    try:
        from agent_first_browse.survey.site_quirks import apply_site_quirks_to_action
    ...
    if verb in ("click", "type"):
        if (
            ...
            from agent_first_browse.actions.tools import mcp_ground_action
    ...
    try:
        ...
        if intent_journal_enabled():
            from agent_first_browse.memory.intent_journal import make_intent, should_journal, persist_intent
    ...
    if intent_entry is not None:
        try:
            from agent_first_browse.memory.intent_journal import classify_status
    ...
    try:
        ...
        if reality_enabled() and verb in ("click", "type", "press_enter"):
            from agent_first_browse.cognition.reality import (classify_reality, reconcile_with_llm,
...
def _block_done(state: dict, updates: dict, reason: str,
    ...
    """
    from agent_first_browse.verification.outcome import MAX_DONE_BLOCKS
...
async def final_outcome_audit(state: dict, page) -> tuple[bool, str]:
    ...
    from agent_first_browse.verification.outcome import judge_done, should_accept
...
async def _layer_4_cove_check(state: dict, page, updates: dict) -> dict:
    ...
    from agent_first_browse.verification.outcome import judge_done, should_accept, rejection_feedback
    ...
    if verdict is not None:
        ...
        try:
            ...
            if subgoal_lock_enabled() and state.get("prm_checklist"):
                from agent_first_browse.cognition.subgoal_lock import compose_rejection, reconcile_plan_with_ledger
    ...
    from agent_first_browse.verification.safety import cove_pre_done_check
    from agent_first_browse.cognition.core import PlanState, WorkingMemory
...
def _fmt_fail(clean: bool, verb: str, legacy_prefix: str, error: str) -> str:
    ...
    from agent_first_browse.verification.feedback import classify_failure, render_failure
...
async def _execute_action(proposed: dict, page) -> str:
    ...
    try:
        if verb == "goto":
            from agent_first_browse.actions.tools import mcp_navigate
            ...
            from agent_first_browse.actions.tools import mcp_abandon_survey
            boundary_reason = str(proposed.get("survey_boundary_reason") or "")
            fresh_dashboard = boundary_reason == "completed:fresh_dashboard"
            try:
                from agent_first_browse.survey.site_quirks import fresh_dashboard_after_boundary
            ...
            from agent_first_browse.actions.tools import mcp_click
            ...
            from agent_first_browse.actions.tools import mcp_type
            ...
            from agent_first_browse.actions.tools import mcp_set_date_of_birth
            ...
            from agent_first_browse.actions.tools import mcp_scroll
            ...
            from agent_first_browse.actions.tools import mcp_press_enter
            ...
            from agent_first_browse.actions.tools import mcp_wait
