...
try:
    from agent_first_browse.promotion.browser_promoter.state import AgentState, BrowserConfig, CampaignContext
...
try:
    from agent_first_browse.promotion.browser_promoter.dashboard import print_terminal_dashboard, resolve_platform_name
    checks.append(("dashboard imports", True))
except Exception as e:
    checks.append(("dashboard imports", f"FAIL: {e}"))

try:
    from agent_first_browse.promotion.browser_promoter.database import initialize_persistence_database
    checks.append(("database imports", True))
except Exception as e:
    checks.append(("database imports", f"FAIL: {e}"))

try:
    from agent_first_browse.promotion.browser_promoter.worker_planner import WorkerVisionPlanner
    checks.append(("worker_planner imports", True))
except Exception as e:
    checks.append(("worker_planner imports", f"FAIL: {e}"))

try:
    from agent_first_browse.promotion.browser_promoter.browser_runtime import BrowserRuntime
...
try:
    from agent_first_browse.promotion.browser_promoter.nodes import supervisor_node, worker_node, browser_controller_node, router_function
...
try:
    from agent_first_browse.promotion.config import OPENAI_API_KEY, WORKER_VLM_MODEL
    checks.append(("config imports", True))
except Exception as e:
    checks.append(("config imports", f"FAIL: {e}"))

try:
    from agent_first_browse.promotion import __version__
    checks.append((f"promotion version={__version__}", True))
except Exception as e:
